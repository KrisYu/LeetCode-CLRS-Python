...
因为刚刚做完valid parentheses，所以想偷懒，对于n，我们可以产生的string是 2n，每个位置有来两种选择，我们可以先产生2 ^2n个string，然后在里面跳valid的，检查valid 用O(n),复杂度是O(n * 2 ^2n)，可以说是worst case，一定超时。
...
> *小括号串是*一个*递归结构*，跟单链表、二叉树等*递归结构*一样，
>
> 首先想到用*递归*。一步步构造字符串。当左括号出现次数小于指定的n时，可以放置新的左括号。当右括号出现次数小于左括号出现次数时，就可以放置新的右括号